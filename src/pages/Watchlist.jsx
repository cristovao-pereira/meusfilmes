...
export const Watchlist = () => {
    ...
    useEffect(() => {
        filterMovies();
    }, [movies, debouncedSearch, selectedCategories, watchedFilter]);
    ...
    const filterMovies = () => {
        ...
        if (watchedFilter === 'watched') {
            filtered = filtered.filter(movie => movie.assistido === true);
        } else if (watchedFilter === 'unwatched') {
            filtered = filtered.filter(movie => movie.assistido === false || movie.assistido === null);
        }
        ...
        <div className="movies-page">
            <header className={`movies-header ${isScrolled ? 'scrolled' : ''}`}>
                <div className="movies-header-content">
                    <Link to="/watchlist" className="movies-logo">
                        <Film size={32} className="movies-logo-icon" />
                        <span>Meus Filmes</span>
                    </Link>

                    <nav className="movies-nav">
                        <Link to="/watchlist" className="movies-nav-link active">Minha Lista</Link>
            ...
            <MovieDetailsModal
                ...
                onToggleWatched={(id) => {
                    const movie = movies.find((m) => m.id === id);
                    if (movie) {
                        handleToggleWatched(id, !movie.assistido);
                    }
                }}
                onEdit={handleOpenEdit}
                onDelete={(id) => {
